...
export async function POST(req: NextRequest) {
  ...
  const { title, description, price, condition, category, type, phone, location, images, image_url, expected_date, posted_by, staff_id, discount_percent, discount_amount, discount_end_date } = body
  ...
  const { data, error } = await db
    ...
    .insert({
      ...
      discount_amount: isAdmin && discount_amount ? Number(discount_amount) : 0,
...
export async function PUT(req: NextRequest) {
  if (!checkAdmin(req)) return NextResponse.json({ error: 'unauthorized' }, { status: 401 })
  const body = await req.json()
  const { id, title, description, price, cost_price, condition, category, type, phone, location, images, expected_date, posted_by, staff_id, sku, bin_location, status, discount_percent, discount_amount, discount_end_date } = body
