...
export async function GET(req: NextRequest) {
  ...
  if (id) {
    const { data, error } = await db.from('orders').select('*, items(title, price, order_code, images)').eq('id', id).single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json(data)
  }
  const { data, error } = await db.from('orders').select('*, items(title, price, order_code, images)').order('created_at', { ascending: false })
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json(data)
}
...
export async function PATCH(req: NextRequest) {
  ...
  const db = adminClient()
  const { data, error } = await db.from('orders').update(fields).eq('id', id).select('*, items(title, price, order_code, images)').single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  return NextResponse.json(data)
}
